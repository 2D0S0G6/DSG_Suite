...
def test_heuristics_detect_dom_xss_and_secret():
    raw = llm_analysis.analyze(_retriever(), gemini=None)
...
def test_gemini_path_used_when_available(fake_gemini):
    raw = llm_analysis.analyze(_retriever(), gemini=fake_gemini)
    assert any(r["source"] == "gemini-rag" for r in raw)


def test_unavailable_gemini_falls_back_to_heuristics():
    class Down:
        def is_available(self):
            return False

    raw = llm_analysis.analyze(_retriever(), gemini=Down())
